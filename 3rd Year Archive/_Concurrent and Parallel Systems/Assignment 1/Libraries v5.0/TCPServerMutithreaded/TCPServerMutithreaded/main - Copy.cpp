	...
	do {
		...
		if (data.request != "")
		{
			...
			else if (regex_search(data.request, matches, postRX))
			{
				...
				unsigned int postID = topic->nextID++;

				topic->posts.emplace_back(postID, content);

				data.reply = to_string(postID);
			}
			else if (regex_search(data.request, matches, readRX))
			{
				...
				if (topics.find(topicID) != topics.end())
					content = topics[topicID].posts[postID];
				cout << topicID << "\n" << postID << "\n" << content << "\n";
			...
			{
				data.reply = to_string(topics[matches[1]].nextID);
			}
