...
const char* EasyGraphics::EASY_GRAPHICS_VERSION_STRING="[EasyGraphics Version 2.0]";
...
EasyGraphics::EasyGraphics() : std::ostream(this), noofrows(20), noofcols(20), textcolour(WHITE), hdcback(NULL), immediatemode(true), bitmapcache(16), pencache(16), brushcache(16)
{
...
void EasyGraphics::setFont(int pointsize, const wchar_t* typeface)
{
  LOGFONT logfont;
  logfont.lfHeight=-MulDiv(pointsize, GetDeviceCaps(hdcback, LOGPIXELSY), 72);
  logfont.lfWidth=0;
  ...
  int len=(typeface?(int) wcslen(typeface):0);
  if (len>31) // make sure the typeface name is less than the max buffer length to avoid buffer overrun
    len=31;
  memcpy(logfont.lfFaceName, typeface, sizeof(wchar_t)*len);
  logfont.lfFaceName[len]='\0';

  setHDEFFont(::CreateFontIndirect(&logfont));
}
...
void EasyGraphics::clearScreen(int colour)
{
  curpos.x=0;
  curpos.y=0;
...
void EasyGraphics::drawText(const char* text, int x, int y)
{
  if (text)
  {
    int len=(int) strlen(text)+1;
    wchar_t* tmp=(wchar_t*) malloc(sizeof(wchar_t)*len);
    MultiByteToWideChar(CP_ACP, 0, text, -1, tmp, len);
    drawText(tmp, x, y);
    free(tmp);
  }
}
