...
public class MST
{
	...
	public static void runMST (int datasize, int iter, double[][] result) {
		...
		result = PrimsMST(dataset, timestart, iter, datasize, result);
	...
	public static double[][] PrimsMST(double[][] d, long timestart, int iter, int datasize, double[][] result)
	{
		...
		for(i=0;i<n-1;++i)
		{
			for(j=i+1;j<n;++j) //symmetric array. 0,0 is zero
			{
				//Only non zero edges
		...
		ReadWriteFile.writeResult(result, datasize);
